...
func register(data map[string]string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("peso non valido")
	}
	...
	tokenString, err := token.SignedString([]byte(SECRET_KEY))
	if err != nil {
		return "", fmt.Errorf("error generazione token")
	}
...
func logout(data map[string]string) error {
	...
	if !ok {
		log.Println("ID utente mancante ")
		return fmt.Errorf("user ID mancante")
	}
...
func updateUserData(data map[string]string) (int, error) {
	...
	if data["data"] == "" {
		...
		if err != nil {
			return 2, fmt.Errorf("formato età non valido")
		}
		...
		if err != nil {
			return 2, fmt.Errorf("formato peso non valido")
		}
